...
class Blackjack:
    ...
    def play(self):
        ...
        if p_status == 1:
            ...
            if d_status == 1:
                print("Dealer and you, both got Blackjack! It's a push. (Tie)")
            return 1

        cmd = ""
        while cmd != "Stand":
            bust = 0
            cmd = input("Hit or Stand? (Case sensitive)")

            if cmd == "Hit":
                bust = self.player.hit()
                self.player.show()
            if bust == 1:
                print("You're busted. Good Game!")
                return 1
        print("\n")
        self.dealer.show()
        if d_status == 1:
            print("Dealer got Blackjack! Try harder!")
            return 1
